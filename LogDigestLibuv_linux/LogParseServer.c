...
#define FILENAME_PREFIX "/home/vboxuser/Projects/LogDigestLibuv_linux/output"
...
char current_date[11] = ""; // Store the current date in "YYYY-MM-DD" format
int file_number = 1; // To keep track of the file number for unique filenames
...
void update_date_and_file_number() {
    char new_date[11];
    get_current_date(new_date);

    if (strcmp(current_date, new_date) != 0) {
        strncpy(current_date, new_date, sizeof(current_date));
        file_number = 1;
    }
}
...
void process_xml_and_write(const char *xml_data, size_t xml_data_len) {
    ...
    update_date_and_file_number();

    char filename[MAX_FILENAME_LEN];
    snprintf(filename, sizeof(filename), "%s/output%s.xml", FILENAME_PREFIX, current_date);

    // Create the output directory if it doesn't exist
    struct stat st = {0};
    if (stat(FILENAME_PREFIX, &st) == -1) {
        mkdir(FILENAME_PREFIX, 0700);
    }
